...
{
    [Fact]
    public void Movie_details_page_includes_media_file_issues_content_and_history_actions()
    {
        ...
        Assert.DoesNotContain("@if (_issuesExpanded)", content);
        Assert.DoesNotContain("ToggleIssuesPanel", content);
        Assert.Contains("movie-issues-panel", content);
